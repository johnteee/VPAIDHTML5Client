(function(localTestingEnv) {
  ...
  function isBetrad(url) {
	  if(localTestingEnv)  {
		 return true;
		  ...
		  return /(adsafeprotected.com\/(rjss|rjsi|rfw)\/c.evidon.com|(^https?:)?\/\/c.evidon.com|(^https?:)?\/\/c.betrad.com)/.test(url);
	  }
    //return true;
  }
  ...
  function isInApp(adWidth, adHeight) {
   if (is_mobile) {
       ...
       // otherwise we will run some checks specific to either android or ios to see if we're in an app
        ...
        if (/android/.test(userAgent)) {
            ...
            var widthMatches = (Math.abs(parseInt(adWidth, 10) - doc.clientWidth) < 10);
            var heightMatches = (Math.abs(parseInt(adHeight, 10) - doc.clientHeight) < 5);

            return (widthMatches && heightMatches) ? 1 : 0;
        }
        else {
            // iphone
  ...
  function execute() {
      ...
      for (i = 0; i < _durly_s.length; i++) {
        var durly_script = _durly_s[i];
        // skip if no source exists
        if (!durly_script.src) { continue; }
        // skip when its already been processed
        if (durly_script._durly) { continue; }

        // an akamai url was previously used to serve https resources
        var _durly_src = durly_script.src,
         _durly_own = (isBetrad(_durly_src) || /^https?:\/\/a248.e.akamai.net/.test(_durly_src)); //TODO: check this url...
        ...
        if (_durly_src.match('durly.js') && _durly_own) {
          ...
          var _nw = false, _durly_rnd = Math.floor(Math.random()*100000), _durly_ex = _durly_src.split(';');

          _bao['uqid'] = _durly_rnd;
          _durly_ex = _durly_ex.splice(1, _durly_ex.length);

          for (j = 0; j < _durly_ex.length; j++) {
            if (_durly_ex[j] == 'r=0') { return; }
            var _durly_arg = _durly_ex[j].split('=');
            _bao[_durly_arg[0]] = unescape(_durly_arg[1]);
            if (_durly_arg[0] == 'nowrite') { _nw = true; }
          }
         ...
         // can get the height/width here.  _bao.ad_h and _bao.ad_w
          ...
          if (is_mobile && !_bao["in_app"]) {
            ...
            _bao["in_app"] = isInApp(_bao.ad_w, _bao.ad_h);
          }
          ...
          if (_nw && !is_mobile){
            ...
            if (document.getElementById('ba.js') == null) {
              ...
              mobile_script.setAttribute('src',_durly_url+'/mobile-64.js');
