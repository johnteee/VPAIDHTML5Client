var BAP = ((BAP && BAP.start) ? BAP : false) || (function () {
    var API = {},
		...
		DOMAIN_INFO = 'https://info.betrad.com/',
    ...
    */
    function start(options) {
        ...
        if (options) {
            ...
            options.nid = options.nid.toString().match(/\d/g).join('');
        ...
        try {
            ...
            for (k = 0; k < i.length; k++) {
                ...
                for (j = 0; j < skip.length; j++) {
                    if ((skip[j] == BAP.options[pageId].nid) || (skip[j] == BAP.options[pageId].nid + '|' + pageId) || (skip[j] == ('size|' + BAP.options[pageId].ad_w + 'x' + BAP.options[pageId].ad_h))) {
                ...
                for (j = 0; j < skip.length; j++) {
                    if (skip[j] == ('size|' + BAP.options[pageId].ad_w + 'x' + BAP.options[pageId].ad_h)) {
                ...
                BAP.options[pageId].mini = isMini(BAP.options[pageId].ad_w, BAP.options[pageId].ad_h);
    ...
    function copyOptions(pageId, nid) {
        try {
            ...
            BAP.options[pageId].advLogo = (cud.adv_logo.replace("http:", "https:") || null);
            ...
            cud['default_generic1'] && (BAP.options[pageId].defTrans.generic1 = cud['default_generic1']);
            cud['default_generic2'] && (BAP.options[pageId].defTrans.generic2 = cud['default_generic2']);
            cud['default_generic3'] && (BAP.options[pageId].defTrans.generic3 = cud['default_generic3']);
            cud['default_generic4'] && (BAP.options[pageId].defTrans.generic4 = cud['default_generic4']);
            cud['default_generic5'] && (BAP.options[pageId].defTrans.generic5 = cud['default_generic5']);
            cud['default_generic6'] && (BAP.options[pageId].defTrans.generic6 = cud['default_generic6']);
            cud['default_link1'] && (BAP.options[pageId].defTrans.link1 = cud['default_link1']);
            cud['default_link2'] && (BAP.options[pageId].defTrans.link2 = cud['default_link2']);
            cud['default_link2'] && (BAP.options[pageId].defTrans.link3 = cud['default_link3']);
            cud['default_footer'] && (BAP.options[pageId].defTrans.footer = cud['default_footer']);
            // reusing skip flag if the L1 has no appropriate L2, but is not a mini.
            BAP.options[pageId].skipL2 = (cud.skip_L2 || isSkipper(BAP.options[pageId].ad_w, BAP.options[pageId].ad_h));
            // overwrite with localized version if available
            var mp = cud.message_properties || '';
            mp['behavioral_' + country] && (BAP.options[pageId].behavioral = mp['behavioral_' + country]);
            mp['behavioral_' + country] && (BAP.options[pageId].noDefault = true);
            // default icon
            cud['default_icon'] && (!BAP.options[pageId].noDefault) && (BAP.options[pageId].cicon = cud['default_icon']);
            mp['generic_text_' + country] && (BAP.options[pageId].behavioralCustomMessage = mp['generic_text_' + country]);
            mp['adv_name_' + country] && (BAP.options[pageId].advName = mp['adv_name_' + country]);
            mp['adv_msg_' + country] && (BAP.options[pageId].advMessage = mp['adv_msg_' + country]);
            mp['adv_logo_' + country] && (BAP.options[pageId].advLogo = mp['adv_logo_' + country].replace("http:", "https:"));
            mp['adv_link_' + country] && (BAP.options[pageId].advLink = mp['adv_link_' + country]);
            mp['translation_' + country] && (BAP.options[pageId].translation = mp['translation_' + country]);
            mp['translation_' + country] && (BAP.options[pageId].cicon = mp['translation_' + country].icon);
            mp.hasOwnProperty('skip_L2_' + country) && (BAP.options[pageId].skipL2 = mp['skip_L2_' + country]);
            BAP.options[pageId].icon_delay = (cud.icon_delay || 0);
            BAP.options[pageId].icon_display = (cud.icon_display || 'normal');
            BAP.options[pageId].icon_display = (cud.icon_expandable ? 'expandable' : BAP.options[pageId].icon_display);
            BAP.options[pageId].icon_grayscale = (cud.icon_grayscale || 100);
            BAP.options[pageId].container_opacity = (cud.container_opacity || 100);
            // offsets
            BAP.options[pageId].offsetTop = (cud.offset_y ? (!isNaN(parseInt(cud.offset_y)) ? parseInt(cud.offset_y) : 0) : 0);
            BAP.options[pageId].offsetLeft = (cud.offset_x ? (!isNaN(parseInt(cud.offset_x)) ? parseInt(cud.offset_x) : 0) : 0);
            try {
            ...
            if (/_(de|es|nl_be|si|mt|lt|gr|ee|is|bg|tr|il|ar|ar_eg|hr|rs)$/.test(BAP.options[pageId].cicon) || (!BAP.options[pageId].cicon && /de|es|be|si|mt|lt|gr|cy|ee|is|bg|tr|il|sa|eg|hr|rs/.test(country))) {
                //BAP.options[pageId].icon_display = 'expandable';
    ...
    function testMovement() {
        ...
        for (pageId in BAP.options) {
            ...
            if ((!BAP.options[pageId].hidden) && (!BAP.options[pageId].detached) && (pEl)) {
                ...
                pEl = pickChildLevel(pEl, BAP.options[pageId].ad_h, BAP.options[pageId].ad_w);
                if (pEl) {
            ...
            if ((b.dm != 6) && ((el.offsetWidth === 0 && el.offsetHeight === 0) || (getStyle(el, 'display') == 'none'))) {
    ...
    function actionWrite(options, l, ow) {
        dropPixel(protocol + 'l.betrad.com/ct/' + logIdString(options) + [country, l, options.pixel_ad_w, options.pixel_ad_h, 242, options.coid, options.rev].join('/') + '/' + 'pixel.gif?v=' + version + '_' + SCRIPT_VERSION + '&ttid=' + treatment + '&d=' + domain + ow + '&r=' + Math.random());
    }
    ...
    function error(options, ec) {
        ...
        */
        ...
        if (ep[pixel] != ec) {
            ep[pixel] = ec;
            if (pixel) {
                dropPixel(protocol + 'l.betrad.com/ct/' + pixel + 'pixel.gif?e=' + ec + '&v=' + version + '_' + SCRIPT_VERSION + '&d=' + domain + '&r=' + Math.random());
            }
        }
    }
    ...
    function expandIcon(pageId) {
        ...
        $('trigger-box-' + pageId).style.width = BAP.options[pageId].ciconWidth + 'px';
    ...
    function vendor(pageId, v) {
        ...
        if (BAP.options[pageId][v]) {
            p.push(v + '[' + nids[pageId] + ']=' + encodeURIComponent(BAP.options[pageId][v]));
        }
    ...
    function iabLink(pageId, n) {
        var key, mi = DOMAIN_INFO + 'about_behavioral_advertising/section1?n=' + nids[pageId];
        for (key in coveredNotices[pageId]) {
            mi += ',' + key;
        }
    ...
    function isValidElement(el) {
        ...
        if ((el.offsetWidth === 0 && el.offsetHeight === 0) || (getStyle(el, 'display') == 'none')) {
            return false;
        }
    ...
    function proximityDetection(pxId, spotWidth, spotHeight, px) {
        ...
        for (i = 0; i < everything.length; i++) {
            if (nodeAcceptCheck(everything[i])) {
                ...
                if (((img.height == spotHeight) && (img.width == spotWidth)) || ((parseInt(img.getAttribute('height')) == spotHeight) && (parseInt(img.getAttribute('width')) == spotWidth))) {
                    matches[i] = img;
                }
                else {
                    // CSS reparsing.
    ...
    function noticePositionCalculate(pageId) {
        ...
        var posTop, posLeft, pixLeft, pixTop, spotLeft, spotTop
        ...
        else if (/^(1|2|3|4|4.1|4.2|7|8|9)$/.test(BAP.options[pageId].dm)) {
        ...
        if ((BAP.options[pageId].icon_display == 'expandable') || ((BAP.options[pageId].icon_display == 'icon') && (BAP.options[pageId].mini))) {
            if (BAP.options[pageId].positionHorizontal() == 'right') {
                posLeft -= 19;
            }
        }
    ...
    function noticeMode(pageId) {
        var ad, ad2, dm, spotHeight = BAP.options[pageId].ad_h
            , spotWidth = BAP.options[pageId].ad_w
            , px = $('bap-pixel-' + pageId)
            , everything = document.getElementsByTagName("*");
        // find proximityId
        ...
        else if (((BAP.options[pageId].container) && (ad = $(BAP.options[pageId].container))) || ((BAP.options[pageId].check_container == "true") && (ad = px.parentNode))) {
            // native container
            dm = 8;
            if (BAP.options[pageId].check_container == "true") {
                /*jsl:ignore*/
                if ((ifr) && ((ad2 = getAdStandard(frameSize()[0], frameSize()[1])) != 0)) {
                    /*jsl:end*/
    ...
    function noticeVerification(pageId) {
        ...
        if ((BAP.options[pageId].ad) && (!BAP.options[pageId].noticeExists)) {
            if (((BAP.options[pageId].ad.nodeName == 'IFRAME') && (BAP.options[pageId].ad.src in frameNoticed)) ||
                // notice already given for this element
                ((BAP.options[pageId].ad.nodeName == 'EXACT-FRAME') && (frameNoticed.contents))) {
    ...
    function noticeCreate(pageId) {
        var icon, iconWidth, opacity, z = ''
            , expansion = ''
            , div = $('BAP-holder')
            , click = 'BAP.action(\'' + pageId + '\', \'S\'); BAP.createL2(\'' + pageId + '\');'
            , iconDir = (BAP.options[pageId].icon == 'g' ? '/icong' : '/icon');
        if (!div) {
        ...
        else if ((BAP.options[pageId].icon_display == 'icon') && (BAP.options[pageId].mini)) {
            icon = '<img src="' + DOMAIN_ROOT + iconDir + '/' + micon + '.png">';
            iconWidth = 19;
        }
        else {
        ...
        if (BAP.options[pageId].mini || BAP.options[pageId].skipL2) {
            click = 'BAP.action(\'' + pageId + '\', \'S\'); BAP.action(\'' + pageId + '\', \'M\'); window.open(BAP.moreInfoHref(' + pageId + '), \'_newtab\');';
        }
        ...
        if (BAP.options[pageId].amzn) {
            // amazon L2
            click = 'BAP.action(\'' + pageId + '\', \'S\'); window.open(\'http://www.amazon.com/gp/dra/info/?pn=1&pg=daaedisc&pp=1,e,' + BAP.options[pageId].coid + ',' + BAP.options[pageId].nid + '\', \'_newtab\');';
        }
        icon = '<span id="trigger-' + pageId + '" style="' + z + 'position:absolute;" class="bap-trigger" onclick="' + click + '"' + expansion + '>' + icon + '</span>';
    ...
    function handleMessage(e) {
        try {
            var f, i, r, nid, key, ad, pageId, frames, data = e;
            if (e.data) {
                data = e.data;
            }
            ...
            if (message == 'BAPFRAMEBROADCAST') {
                ...
                for (pageId in BAP.options) {
                    ...
                    if ((ad) && (((ad.nodeName == 'OBJECT') || (ad.nodeName == 'EMBED')) && (f == url) && (!BAP.options[pageId].noticeExists)) || ((ad.nodeName == 'EXACT-FRAME'))) {
                        // notify that there is a match in the stack and alert for the covered nid
                        try {
                ...
                for (pageId in BAP.options) {
                    $('trigger-' + pageId).onclick = function () {
                        action(pageId, 'S');
                        window.open('http://www.amazon.com/gp/dra/info/?pn=1&pg=daaedisc&pp=1,e,' + BAP.options[pageId].coid + ',' + BAP.options[pageId].nid, '_newtab');
                    };
                }
            }
        }
    ...
    function updateL2(pageId) {
        ...
        if (browser.QuirksMode && BAP.options[pageId].popupWidth && (popup.style.display != 'none')) {
    ...
    function createPopupLayer(pageId) {
        ...
        if (BAP.options[pageId].behavioral == 'definitive') {
            ...
            if (BAP.options[pageId].advName) {
                generic_msg += '<br><br>' + BAP.options[pageId].server.name + ' ' + sg3 + ' ' + BAP.options[pageId].advName + ' ' + sg4;
            }
        }
            ...
            generic_msg = sg2;
            if (BAP.options[pageId].advName) {
                generic_msg += '<br><br>' + BAP.options[pageId].server.name + ' ' + sg3 + ' ' + BAP.options[pageId].advName + ' ' + sg5;
            }
        }
        ...
        if (reg == 1) {
            ...
            qw = browser.QuirksMode ? 'width:296px !important;' : 'width:auto !important;max-width:299px;min-width:276px;';
            noticeHTML = '<div id="bap-notice-' + pageId + '" class="bap1 bap-notice" style="' + qw + ';display:none;' + z + '"><div class="bap-div"><div class="bap-close" onclick="BAP.toggle(' + pageId + ');return false;">' + sc + '</div><div class="bap-img-container">' + (BAP.options[pageId].advLogo ? ((BAP.options[pageId].advLink && !BAP.options[pageId].hideCustom) ? '<a class="bap-blue" target="_blank" href="' + BAP.options[pageId].advLink + '" onclick="BAP.action(\'' + pageId + '\', \'A\');"><img id="bap-logo-' + pageId + '" border="0"></a>' : '<img id="bap-logo-' + pageId + '" border="0">') : '') + '</div><p>' + generic_msg + '</p><div class="bap-link-div"' + lds + '><a class="bap-blue" href="about:blank" id="bap-link-1-' + pageId + '" target="_blank" onclick="BAP.action(\'' + pageId + '\', \'M\');" onmouseover="BAP.moreInfoLink(\'' + pageId + '\')">' + sm + ' &raquo;</a></div>' + (BAP.options[pageId].hideWhatIs ? '' : '<div class="bap-link-div"' + lds + '><a class="bap-blue" href="about:blank" id="bap-link-2-' + pageId + '" target="_blank" onclick="BAP.action(\'' + pageId + '\', \'B\');" onmouseover="BAP.iabLink(\'' + pageId + '\')">' + sw + ' &raquo;</a></div>');
            if (!BAP.options[pageId].hideCustom) {
                if (BAP.options[pageId].advLink && BAP.options[pageId].advMessage) {
                    noticeHTML += '<div class="bap-link-div"' + lds + '><a class="bap-blue" target="_blank" href="' + BAP.options[pageId].advLink + '" onclick="BAP.action(\'' + pageId + '\', \'A\');">' + BAP.options[pageId].advMessage + ' &raquo;</a></div>';
                }
                else if (BAP.options[pageId].advMessage) {
                    noticeHTML += '<div class="bap-link-div"' + lds + '>' + BAP.options[pageId].advMessage + '</div>';
                }
            }
            noticeHTML += '<div class="bap-link-div"' + lds + '>' + se + '</div></div></div>';
        }
        else if (reg == 2) {
            noticeHTML = '<div id="bap-notice-' + pageId + '" class="bap2 bap-notice" style="width:156px;display:none;' + z + '"><div class="bap-div"><div class="bap-close" onclick="BAP.toggle(BAP.$(\'bap-notice-' + pageId + '\'));return false;">' + sc + '</div><div class="bap-img-container">' + (BAP.options[pageId].advLogo ? ((BAP.options[pageId].advLink && !BAP.options[pageId].hideCustom) ? '<a class="bap-blue" target="_blank" href="' + BAP.options[pageId].advLink + '" onclick="BAP.action(\'' + pageId + '\', \'A\');"><img id="bap-logo-' + pageId + '" border="0"></a>' : '<img id="bap-logo-' + pageId + '" border="0">') : '') + '</div><p>' + generic_msg + '</p><div class="bap-link-div"><a class="bap-blue" href="about:blank" id="bap-link-1-' + pageId + '" target="_blank" onclick="BAP.action(\'' + pageId + '\', \'M\');" onmouseover="BAP.moreInfoLink(\'' + pageId + '\')">' + sm + ' &raquo;</a></div>' + (BAP.options[pageId].hideWhatIs ? '' : '<div class="bap-link-div"><a class="bap-blue" href="about:blank" id="bap-link-2-' + pageId + '" target="_blank" onclick="BAP.action(\'' + pageId + '\', \'B\');" onmouseover="BAP.iabLink(\'' + pageId + '\')">' + sw + ' &raquo;</a></div>');
            if (!BAP.options[pageId].hideCustom) {
                if (BAP.options[pageId].advLink && BAP.options[pageId].advMessage) {
                    noticeHTML += '<div class="bap-link-div"><a class="bap-blue" target="_blank" href="' + BAP.options[pageId].advLink + '" onclick="BAP.action(\'' + pageId + '\', \'A\');">' + BAP.options[pageId].advMessage + '&nbsp;&raquo;</a></div>';
                }
                else if (BAP.options[pageId].advMessage) {
                    noticeHTML += '<div class="bap-link-div">' + BAP.options[pageId].advMessage + '</div>';
                }
            }
            noticeHTML += '<div class="bap-link-div">' + se + '</div></div></div>';
        }
        else if (reg == 5) {
            noticeHTML += '<div id="bap-notice-' + pageId + '" class="bap5 bap-notice" style="display:none;' + z + '"><div class="bap-div"><div class="bap-close" onclick="BAP.toggle(BAP.$(\'bap-notice-' + pageId + '\'));return false;">' + sc + '</div><p>' + generic_msg + ' <a class="bap-blue-link" href="about:blank" id="bap-link-1-' + pageId + '" target="_blank" onclick="BAP.action(\'' + pageId + '\', \'M\');" onmouseover="BAP.link(\'bap-link-1-' + pageId + '\', \'' + DOMAIN_INFO + 'more_info/' + nids[pageId] + '\')">' + sl + '</a></p><div class="bap-link-div">' + se + '</div></div></div>';
        }
        else if (reg == 6) {
            qw = browser.QuirksMode ? 'width: 728px !important;' : 'min-width: 675px !important; width: auto !important;';
            lds = browser.QuirksMode ? ' style="width:275px !important; height: 20px;"' : '';
            // push everything down for single link
            if ((BAP.options[pageId].hideCustom) && (BAP.options[pageId].hideWhatIs)) {
                noticeHTML += '<style>#bap-first-link-div { padding-top:45px !important; }</style>';
            }
            else if ((BAP.options[pageId].hideCustom) || (BAP.options[pageId].hideWhatIs)) {
                noticeHTML += '<style>#bap-first-link-div { margin-top: 20px !important; }</style>';
            }
            noticeHTML += '<div id="bap-notice-' + pageId + '" class="bap6 bap-notice" style="display:none;' + qw + z + '"><div class="bap-div"><div id="bap-gradient-1" class="bap-gradient"><div id="bap-first-link-div" class="bap-link-div"' + lds + '><a class="bap-blue" href="about:blank" id="bap-link-1-' + pageId + '" target="_blank" onclick="BAP.action(\'' + pageId + '\', \'M\');" onmouseover="BAP.moreInfoLink(\'' + pageId + '\')">' + sm + ' &raquo;</a></div>';
            noticeHTML += (BAP.options[pageId].hideWhatIs ? '' : '<div class="bap-link-div"' + lds + '><a class="bap-blue" href="about:blank" id="bap-link-2-' + pageId + '" target="_blank" onclick="BAP.action(\'' + pageId + '\', \'B\');" onmouseover="BAP.iabLink(\'' + pageId + '\')">' + sw + ' &raquo;</a></div>');
            if (!BAP.options[pageId].hideCustom) {
                if (BAP.options[pageId].advLink && BAP.options[pageId].advMessage) {
                    noticeHTML += '<div class="bap-link-div"' + lds + '><a class="bap-blue" target="_blank" href="' + BAP.options[pageId].advLink + '" onclick="BAP.action(\'' + pageId + '\', \'A\');">' + BAP.options[pageId].advMessage + ' &raquo;</a></div>';
                }
            ...
            noticeHTML += '<div class="bap-link-div"' + lds + '>' + se + '</div></div><div id="bap-gradient-2" class="bap-gradient"><div class="bap-close" onclick="BAP.toggle(BAP.$(\'bap-notice-' + pageId + '\'));return false;">' + sc + '</div><div class="bap-img-container">' + (BAP.options[pageId].advLogo ? ((BAP.options[pageId].advLink && !BAP.options[pageId].hideCustom) ? '<a class="bap-blue" target="_blank" href="' + BAP.options[pageId].advLink + '" onclick="BAP.action(\'' + pageId + '\', \'A\');"><img id="bap-logo-' + pageId + '" border="0"></a>' : '<img id="bap-logo-' + pageId + '" border="0">') : '') + '</div><p>' + generic_msg + '</p></div></div></div>';
		}
        if (!div) {
            div = document.createElement('div');
            div.setAttribute('id', 'BAP-holder');
    ...
    function _offset(elem) {
        ...
        var body = doc.body
            ...
            , clientTop = docElem.clientTop || (browser.QuirksMode ? body.clientTop : 0) || 0
    ...
    var BAPUtil = { /* NON_PROD */
        ...
        , /* NON_PROD */
        css: function (reg) { /* NON_PROD */
            if ((reg) && (!$('bass-' + reg))) { /* NON_PROD */
                ...
                ss.setAttribute('href', DOMAIN_CSS + reg + '.css?r=' + Math.random()); /* NON_PROD */
